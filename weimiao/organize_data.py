...
class OrganizeData:
    ...
    def get_basic_data(self, filename, col=2):
        """
            ...
            注：abs() 将 负数 转成 正数
        """
        special_field_list = ["财务费用", "现金及现金等价物净增加额", "期末现金及现金等价物余额",
        ...
        if filename:
            ...
            for row_num in range(sheet.nrows):
                ...
                if field in special_field_list:
    ...
    def import_cashflow_sheet(self):
        ...
        # 整理需要导入的字段 {字段名:行数}
        field_dict = {"销售商品、提供劳务收到的现金": 5, "营业收入": 6, "归属于母公司股东的净利润": 13,
                      "经营活动产生的现金流量净额": 19,"购建固定资产、无形资产和其他长期资产支付的现金": 25,
                      "分配股利、利润或偿付利息支付的现金": 32, "投资活动产生的现金流量净额": 40,
                      "筹资活动产生的现金流量净额": 41, "现金及现金等价物净增加额": 47, "期末现金及现金等价物余额": 54}
