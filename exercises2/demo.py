...
# http://192.168.23.250:5678/input_num?num=100
...
def get_request():
    ...
    if not num.isdigit() or not int(num) > 1:
        return '请输入大于1的数字！'

    # 将该数字范围内的所有质数保存入list中
    # prime_number_list = []
    # for i in range(1, int(num) + 1):
    #     if is_prime_number(i):
    #         prime_number_list.append(i)
    prime_number_list = [i for i in range(2, int(num) + 1) if is_prime_number(i)]

    # 将list转换成string
    prime_number_str = ",".join([str(x) for x in prime_number_list])
    print prime_number_str

    return '从1~' + str(num) + '内的所有质数为：' + prime_number_str
...
# http://192.168.23.250:5678/analyze_idCard?id_card=310102198602114417
@app.route('/analyze_idCard', methods=['get'])
def analyze_id_card():
...
if __name__ == "__main__":
    ...
    # analyze = AnalyzeIDCard("310110198502114417")
    # print analyze.analyze_id_card()
